...
module.exports = class WantManager {
  ...
  _sendResponse (peerId, msg) {
    if (!this.peers.has(peerId.toB58String())) {
      return new Error('peer is not available in WantManager')
    }
  ...
  _startPeerHandler (peerId) {
    ...
    const msg = new Message({eth: 'placeholder_eth_addr'})
  ...
  start (callback) {
    ...
    //   // const fullwantlist = new Message(true)
    //   const msg = new Message({eth: 'eth_placeholder_addr'})
    //   // this.wantlist.forEach((entry) => {
    //   //   fullwantlist.addEntry(entry.cid, entry.priority)
    //   // })
    //
    //   this.peers.forEach((p) => p.addMessage(msg))
    // }, 60 * 1000)
